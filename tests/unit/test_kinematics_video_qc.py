...
from vslp.analysis.kinematics.video_qc import run_video_qc, write_video_qc_framework_placeholder
...
def test_run_video_qc_writes_summary(tmp_path: Path):
    ...
    assert Path(res["framework_placeholder_csv"]).exists()
    assert Path(res["framework_placeholder_json"]).exists()
    placeholder = pd.read_csv(res["framework_placeholder_csv"])
    assert "placeholder_to_be_built_in" in set(placeholder["status"])


def test_write_video_qc_framework_placeholder_is_explicit(tmp_path: Path):
    paths = write_video_qc_framework_placeholder(tmp_path)
    assert paths["placeholder_csv"].exists()
    assert paths["placeholder_json"].exists()
    text = paths["placeholder_json"].read_text(encoding="utf-8")
    assert "PLACEHOLDER_TO_BE_BUILT_IN" in text
    assert "not a completed QC framework" in text
...
def test_kinematics_gui_exposes_video_qc_stage():
    ...
    assert "QC framework placeholder" in app_py
    assert "QC TO BE BUILT IN" in app_py
