...
from vslp.acoustic.features.plugins.audio_utils import read_region_audio
...
class ResonatoryBaselinePlugin(AcousticFeaturePlugin):
    ...
    def compute(self, context: FeatureContext) -> dict[str, FeatureValue]:
        ...
        x, _sr, region_note = read_region_audio(context.segmentation_wav_path, context.segments_csv, region=context.analysis_region, min_pause_duration_sec=min_pause)
        if x.size == 0:
            return {"RMSamp": FeatureValue("RMSamp", np.nan, "failed", "empty_audio")}
        return {"RMSamp": FeatureValue("RMSamp", float(np.sqrt(np.mean(x ** 2))), "computed", "rms_amplitude_from_selected_region; " + region_note)}
